...
data_m = np.loadtxt('MNT30cmElorn2010_G1.txt')
#print(data_m[:10])
...
#sc = plt.scatter(output_dataframe[0], output_dataframe[1], c=output_dataframe['différence profondeur'], cmap='seismic', vmax =1,vmin =-1)
...
interp_triangulaire2 = interp_triangulaire.values[~np.isnan(interp_triangulaire.values)] #on enlève les valeurs nan !!! uniquement pour tracer boxplot et histogramme !!!
...
# plt.figure()
# plt.pcolormesh(interp_nearneighbor.x,interp_nearneighbor.y,interp_nearneighbor.values,cmap='gist_earth',shading='flat')
